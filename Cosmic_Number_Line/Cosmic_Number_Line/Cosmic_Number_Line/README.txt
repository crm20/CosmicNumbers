...
-----------------------------------------------------------------------------------------------------------------------------
Creating a tutorial:
1. Creating the storyboard tutorial scene:
   There are multiple ways to do this, but the easiest way to do it is just copy + paste the level scene and replacing your level view controller with it. This way, you keep the necessary IBOutlets, constraints, etc. 
    For example, if you're creating a level five tutorial, copy and paste the Level Five Game Scene. You can do this by clicking the scene itself and either the typical Command+C --> Command+V or going to Edit and clicking Copy/Paste. 
   Once you do this, you also need to reconnect it to the other parts of the storyboard, so add the required segues/unwinds as described by David.
   
2. Determine your goals and subgoals:
   Figuring out the objectives beforehand will give you a better idea of how you want your tutorial's transition phases to go.
   Once you figure out the number of goals and number of subgoals, it'll be interpreted by the variable [stages] and [subGoals] in code.
   For example, if you want the player to move Tommy onto the number line, that is considered as a stage. In order to do that, the player needs the possible following subgoals: 
a) know where Tommy is,
b) how to drag Tommy,
c) etc.

3. Look at TutorialExample.swift:
   TutorialExample.swift contains the skeletal structure for the tutorials. It contains variables and functions that are going to generally be in the tutorials and comments that describe what their purposes.
